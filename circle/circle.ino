...
void DrawCircle(uint16_t row, uint16_t col, uint32_t c, uint8_t wait) {
  static int xx[] = {-1,0,1,2,2,2,1,0,-1,-2,-2,-2};
  static int yy[] = {2,2,2,1,0,-1,-2,-2,-2,-1,0,1};
  for (int b = 0; b < 100; b++) {
    for (int dot=0; dot < ARRAY_LEN(xx); dot++) {
      SetPixelMatrix(xx[dot]+row, yy[dot]+col, strip.Color(0, 0, 2*(50-abs(50-b))));
    }
    strip.show();
    delay(wait);
  }
